...
function _convert_dollar_name_to_monitor_name(monitor_name) {
  if (
    monitor_name.split(" ")[0] === "Dólar" &&
    monitor_name !== "Dólar Today"
  ) {
...
async function getMonitor(monitorCode, nameProperty, prettify) {
  ...
  try {
    if (response.status !== 200) {
      throw new Error(
        `Error de comunicación CriptoDolar. Codigo: ${response.status}`
      );
    }
    ...
    for (const monitor of jsonResponse) {
      if (
        monitor.type.includes("bolivar") ||
        monitor.type.includes("bancove")
      ) {
        const result = {
    ...
    try {
      ...
      if (nameProperty) {
        ...
        if (value === undefined) {
          throw new Error(
            "Consulte la documentación de la biblioteca: https://github.com/fcoagz/consulta-dolar-venezuela"
          );
        } else {
